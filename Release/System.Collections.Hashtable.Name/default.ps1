...
Task Copy   -description "Copy items to the release folder" `
    ...
    if ($target.Type -ieq "Module" ){
        Write-verbose "Copy Module: $BasePath --> $ReleaseDir\$target.Name" -Verbose              
        MD $ReleaseDir -ErrorAction SilentlyContinue | Out-Null
        MD "$PublishDir\$target.Name" -ErrorAction SilentlyContinue | Out-Null
        &robocopy $BasePath $PublishDir\$target.Name  /s /xf $($Exclude -join ' ') /xd .git Release Scratch
