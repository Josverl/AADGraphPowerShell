...
Describe "AADGraph : tenant test" {
    ...
    it "Connect to tenant $tenant" {
        {Connect-AAD -tenant $tenant -Credentials $cred  }  | Should not Throw    
    }

    Context 'Object paging 10' { 
        it "gets first page of Objects" { {$Script:Users_P1 = Get-AADObject -Type 'users'  -PageSize 10 }| Should not throw
            $Script:Users_P1.Count | Should be 10
        }

        it "gets Next page of Objects " { {$Script:Users_P2 = Get-AADObject -Type 'users' -Next -PageSize 10}| Should not throw
    ...
    Context 'Object paging default' { 
        ...
        it "gets first page of Objects" { {$Script:Users_P1 = Get-AADObject -Type 'users'   }| Should not throw
            $Script:Users_P1.Count | Should be 100
        }

        it "gets Next page of Objects " { {$Script:Users_P2 = Get-AADObject -Type 'users' -Next }| Should not throw
    ...
    Context 'User paging'  { 
        ...
        it "gets first page of users" { {$Script:Users_P1 = Get-AADUser }| Should not throw
            $Script:Users_P1.Count | Should be 100
        }

        it "gets Next page of users " { {$Script:Users_P2 = Get-AADUser -Next}| Should not throw
            $Script:Users_P2.Count | Should be 100
        }
